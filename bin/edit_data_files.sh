...
#    sed 's/\s{3,}/\n/g' $file | tail -n +2 | sed 's/""//g' | sed 's/; ;/;;/g' | sed 's/; ;/;;/g' | egrep -v '^\s' >$newfile #remove header and blank line from data file
    bomstrip $file | sed 's/  .//g' | egrep -v '^\s' | sed 's/\^ \^/\^\|\^/g' | sed 's/\r/''/g' | sed 's/\^\^//g' >$newfile #remove bom, end of line ws from data file, change delimiter to pipe, clean up line endings, remove empty quoted fields
done
